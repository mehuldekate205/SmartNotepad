...
import os
import threading
from ShowRuntime import ElapsedTimeClock


class Notepad(threading.Thread):
    def __init__(self):
        super().__init__()
        # self.new_file = None
        self.root = tk.Tk()
    ...
    def set_tool_bar(self):
        ...
        clock = ElapsedTimeClock(self.tool_bar)
        clock.grid(row=0, column=10, padx=3)
    ...
    def set_status_bar(self):
        self.status_bar = ttk.Label(self.root, text=f'Characters : 0   Words : 0   Date : {datetime.datetime.now():%a, %b %d %Y}')
    ...
    def set_file_menu_event_bindings(self):
        ...
        self.root.bind("<Control-q>", lambda e: self.say_something())
        self.root.bind("<Control-Q>", lambda e: self.say_something())
    ...
    def set_canvas(self):
        ...
        self.text_editor.config(yscrollcommand=self.scroll_bar.set,font=(self.current_font_family, self.current_font_size))
    ...
    def changed(self, event=None):
        if self.text_editor.edit_modified():
            ...
            characters = len(self.text_editor.get(1.0, tk.END))
            self.status_bar.config(text=f'Characters : {characters}   Words : {words}   Date : {datetime.datetime.now():%a, %b %d %Y}')
    ...
    def say_something(self):
        try:
            ...
            if self.takeAudio == "":
                ...
                self.find_func()
            elif self.takeAudio.lower() == "hey mojo hide statusbar" or self.takeAudio.lower() == "hey mojo hide status bar":
                self.hide_statusbar()
    ...
    def change_bold(self):
        ...
        if self.text_property.actual()['slant'] == 'italic':
            if self.text_property.actual()['weight'] == 'normal':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'bold', 'italic'))
            if self.text_property.actual()['weight'] == 'bold':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'normal', 'italic'))
        else:
            if self.text_property.actual()['weight'] == 'normal':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'bold', 'roman'))
            if self.text_property.actual()['weight'] == 'bold':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'normal', 'roman'))

    def change_italic(self):
        self.text_property = tk.font.Font(font=self.text_editor['font'])
        if self.text_property.actual()['weight'] == 'bold':
            if self.text_property.actual()['slant'] == 'roman':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'italic', 'bold'))
            if self.text_property.actual()['slant'] == 'italic':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'roman', 'bold'))
        else:
            if self.text_property.actual()['slant'] == 'roman':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'italic', 'normal'))
            if self.text_property.actual()['slant'] == 'italic':
                self.text_editor.configure(font=(self.current_font_family, self.current_font_size, 'roman', 'normal'))
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            self.file_dialogue.geometry('500x450+500+200')
